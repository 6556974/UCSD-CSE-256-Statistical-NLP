...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            # Choose to visualize only one attention head, for example, head 0
            att_map = attn_map[0, 0].detach().cpu().numpy()  # Remove batch and select head 0

            # Check if the attention probabilities sum to 1 over rows
            total_prob_over_rows = torch.sum(attn_map[0], dim=1).mean(dim=1)[0]
            if torch.any(total_prob_over_rows < 0.9) or torch.any(total_prob_over_rows > 1.1):
